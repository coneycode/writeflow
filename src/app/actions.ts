...
import { ensureDirectory, projectRoot, projectsRoot, slugifyProjectName } from "@/lib/paths";
...
import type { FinalVariantSelection } from "@/schemas/final-selection";
...
function resolveMemoryTarget(rootPath: string, target: string) {
  const requested = target.replace(/\\/g, "/").replace(/^\/+/, "");
  const normalized = requested.startsWith("memory/canon/world/") && requested.endsWith(".md") && !requested.includes("..")
    ? "memory/canon/world.md"
    : requested;
...
export async function listProjectArtifacts(projectId: string, kind?: "direction" | "outline" | "variant_strategy" | "draft" | "edit" | "review" | "final_selection" | "selected_final" | "memory_patch" | "chapter_plan" | "context_archive") {
...
export async function listVariantStrategyArtifacts(projectId: string) {
  const project = await getProject(projectId);
  if (!project) {
    return [];
  }

  const artifacts = await listProjectArtifacts(projectId, "variant_strategy");
  return Promise.all(
    artifacts.map(async (artifact) => ({
      artifact,
      data: await readJsonArtifact<VariantStrategyPlan>(project.rootPath, artifact.filePath),
    })),
  );
}
...
export type ChapterArchive = {
  ...
  manuscript: string;
  selectionRationale?: FinalChapter["selectionRationale"];
  direction: { option: DirectionOption | null; recommendation: string } | null;
  outline: BeatSheet | null;
  variantStrategy: VariantStrategyPlan | null;
  draft: DraftSet | null;
  edit: EditSet | null;
  reviews: ChapterArchiveReview[];
  finalSelection: FinalVariantSelection | null;
};
...
export async function getChapterArchive(projectId: string): Promise<ChapterArchive[]> {
  ...
  return Promise.all(
    chapters.map(async (chapter, index): Promise<ChapterArchive> => {
      ...
      const variantStrategyArtifact = draftArtifact?.parentArtifactId ? await getArtifactById(projectId, draftArtifact.parentArtifactId) : null;
      const variantStrategy = variantStrategyArtifact?.kind === "variant_strategy" ? await readJsonArtifact<VariantStrategyPlan>(project.rootPath, variantStrategyArtifact.filePath) : chapter.selectionRationale?.strategyPlan ?? null;

      const outlineParentId = variantStrategyArtifact?.kind === "variant_strategy" ? variantStrategyArtifact.parentArtifactId : draftArtifact?.parentArtifactId;
      const outlineArtifact = outlineParentId ? await getArtifactById(projectId, outlineParentId) : null;
      ...
      const finalSelectionArtifact = latest.parentArtifactId && index === chapters.length - 1 ? await getArtifactById(projectId, latest.parentArtifactId) : null;
      const finalSelection = chapter.selectionRationale?.finalSelection ?? (finalSelectionArtifact?.kind === "final_selection" ? await readJsonArtifact<FinalVariantSelection>(project.rootPath, finalSelectionArtifact.filePath) : null);
      ...
      return {
        ...
        selectionRationale: chapter.selectionRationale,
        direction,
        outline,
        variantStrategy,
        draft,
...
export async function deleteProject(formData: FormData) {
  ...
  if (!normalizedRoot.startsWith(normalizedProjectsRoot + path.sep)) {
    throw new Error("Project root escapes the projects directory.");
  }

  const runs = await db.select({ id: schema.runs.id }).from(schema.runs).where(eq(schema.runs.projectId, project.id));
  for (const run of runs) {
    await db.delete(schema.gates).where(eq(schema.gates.runId, run.id));
    await db.delete(schema.runSteps).where(eq(schema.runSteps.runId, run.id));
  }
  await db.delete(schema.artifacts).where(eq(schema.artifacts.projectId, project.id));
  await db.delete(schema.runs).where(eq(schema.runs.projectId, project.id));
...
async function runScribeVariant(input: {
  ...
  for (const [sceneIndex, scene] of input.beatSheet.scenes.entries()) {
    ...
    const segment = await runScribeSegment({
      ...
      label: `${prefix}变体 ${input.variant.id} · 第 ${sceneIndex + 1} 场 / 共 ${sceneTotal} 场：${scene.title}`,
    });
  ...
  return {
    ...
    strategy: input.variant.goal,
    strategyPlan: input.variant,
    strengths: ["按动态候选策略分段生成，降低长正文请求超时风险。", ...input.variant.emphasis],
    risks: [input.variant.risk, "场景之间由分段上下文衔接，生成后需重点检查转场顺滑度。"],
    segments,
  ...
  await startJob({
    ...
    executor: async () => {
      ...
      const variantRequests = [
        { id: "A" as const, title: "紧凑推进版", goal: "紧凑推进：强调悬念、行动压力和场景钩子。", emphasis: ["悬念", "行动压力", "场景钩子"], avoid: ["拖慢主线"], bestFor: "需要快速推进章节事件。", risk: "可能牺牲人物余波。" },
        { id: "B" as const, title: "情绪压迫版", goal: "情绪压迫：强调人物感受、关系张力和细节余波。", emphasis: ["人物感受", "关系张力", "细节余波"], avoid: ["空转抒情"], bestFor: "需要强化人物代价和关系压力。", risk: "可能显得推进不足。" },
      ] satisfies VariantStrategy[];
...
function reviewCandidates(source: DraftSet | EditSet) {
  return source.variants.map((variant) => ({
    ...
    title: variant.title,
    strategy: "strategy" in variant ? variant.strategy : variant.editStrategy,
...
${JSON.stringify(
  {
    ...
    title: input.variant.title,
    strategy: input.variant.strategy,
    strategyPlan: input.variant.strategyPlan,
  },
  null,
  2,
)}
...
async function diagnoseReviewFailure(input: {
  projectName: string;
  manuscriptContext: string;
  recall: string;
  editSet: EditSet;
...
Reviewed variants and excerpts:
${JSON.stringify(input.editSet.variants.map((variant) => ({ id: variant.id, title: variant.title, strategyPlan: variant.strategyPlan, excerpt: variantExcerpt(variant.manuscript) })), null, 2)}

Latest reviews:
${JSON.stringify(input.reviews, null, 2)}

Unresolved review issues:
${input.issues.map((issue, index) => `${index}. [${issue.severity}] ${issue.variantId ?? "all"} ${issue.location ?? ""}\n问题：${issue.problem}\n证据：${issue.evidence}\n建议：${issue.suggestedFix}`).join("\n\n")}

Diagnose whether these unresolved issues are auto-fixable narrative defects, true author-intent choices, system issues, or unknown. Do not write prose.`;
...
async function applyTargetedRepair(input: {
  projectName: string;
  manuscriptContext: string;
  recall: string;
...
Variant: ${input.variant.id} — ${input.variant.title}

Unresolved issues for this variant:
${formatIssuesForPrompt(input.issues)}
...
Apply the targeted repair and return the complete revised manuscript.`;

  return runAgent({ agent: agents.targetedRepair, prompt, maxTokens: revisionMaxTokens(input.variant.manuscript), label: `定点修复 变体 ${input.variant.id}` });
}

async function verifyTargetedRepair(input: {
  projectName: string;
  manuscriptContext: string;
  recall: string;
  variant: EditedVariant;
  ...
  diagnosis: ReviewFailureDiagnosis;
  repairPlan: TargetedRepairPlan;
  repairResult: TargetedRepairResult;
}) {
  const prompt = `Project: ${input.projectName}

续写上文末尾:
${tailText(input.manuscriptContext, 1200)}
...
Before excerpt:
${excerptText(input.originalManuscript, 5000)}

After excerpt:
${excerptText(input.repairResult.manuscript, 5000)}

Changes made:
${input.repairResult.changesMade.map((item) => `- ${item}`).join("\n")}

Verify whether the targeted repair solved the original issue without obvious regressions. Do not rewrite prose.`;

  return runAgent({ agent: agents.repairVerifier, prompt, maxTokens: 1200, label: `验证定点修复 ${input.variant.id}` });
}

function verificationFailed(verification: RepairVerification) {
  return !verification.issueResolved || verification.resolutionConfidence === "low" || verification.introducedRegressions.length > 0;
}
...
- 允许的目标文件只有：memory/canon/timeline.md、memory/canon/world.md、memory/progress/open_threads.md、memory/progress/state.md、memory/style/voice.md、memory/style/taboo.md、memory/canon/characters/*.md。不要发明 memory/canon/world/military.md 这类嵌套 world 文件；军事/地点/制度等世界设定统一写入 memory/canon/world.md。
...
Use only these target files: memory/canon/timeline.md, memory/canon/world.md, memory/progress/open_threads.md, memory/progress/state.md, memory/style/voice.md, memory/style/taboo.md, memory/canon/characters/*.md. Do not invent nested world files such as memory/canon/world/military.md; put military/location/world facts into memory/canon/world.md.`;
...
function buildQualityDiagnosis(input: { chapterTitle: string; verdict: string; repairPlan: AutopilotRepairPlan }): AutopilotFailureDiagnosis {
  return {
    title: "审稿多轮修订后仍未通过",
    summary: `《${input.chapterTitle}》已完成自动审稿与自动修订，但复审结论仍为 ${input.verdict}。`,
    cause: input.repairPlan.summary,
    impact: "系统已保留此前定稿章节和本章中间产物；未把未通过版本写入终稿。",
    canRetryAsIs: false,
    recommendedAction: input.repairPlan.rationale,
  };
}

function diagnoseReviewRepairPlan(issues: ReviewIssue[]): AutopilotRepairPlan {
  const materialIssues = issues.filter((issue) => issue.severity !== "minor");
  const relevant = materialIssues.length ? materialIssues : issues;
  const text = relevant.map((issue) => `${issue.location ?? ""} ${issue.problem} ${issue.evidence} ${issue.suggestedFix}`).join("\n").toLowerCase();
  const affectedScenes = Array.from(new Set(relevant.map((issue) => issue.location).filter((item): item is string => Boolean(item)))).slice(0, 6);
  const affectedVariants = Array.from(new Set(relevant.map((issue) => issue.variantId).filter((item): item is string => Boolean(item))));

  if (!materialIssues.length) {
    return {
      level: "review_only",
      confidence: "high",
      summary: "剩余问题主要是 minor 建议或审稿收敛问题，不应默认整章重写。",
      recommendedAction: "rerun_review",
      rationale: "建议优先复审或接受当前最佳版本，避免为非阻塞建议浪费整章生成成本。",
      affectedScenes,
      affectedVariants,
    };
  }

  if (/记忆|设定|canon|关系|时间线|世界观|人物卡|状态/.test(text)) {
    return {
      level: "memory",
      confidence: "medium",
      summary: "主要问题指向记忆/设定一致性，先修正文可能无法解决根因。",
      recommendedAction: "ask_author",
      rationale: "建议先确认或修正设定来源，再从受影响阶段续跑。",
      affectedScenes,
      affectedVariants,
    };
  }

  if (/路线图|本章目标|目标|没有推进|重复|重演|承接|方向|主线|新事件|新信息/.test(text)) {
    return {
      level: "route",
      confidence: "medium",
      summary: "主要问题指向本章目标/路线承接，局部润色通常不能解决。",
      recommendedAction: "regenerate_chapter",
      rationale: "建议回退到本章生成起点，重新生成本章；必要时先调整本章目标。",
      affectedScenes,
      affectedVariants,
    };
  }

  if (/大纲|结构|场景顺序|转折|冲突|节奏结构|铺垫|结尾|中段/.test(text)) {
    return {
      level: "outline",
      confidence: "medium",
      summary: "主要问题出在章节结构或场景安排，整章正文不一定全废。",
      recommendedAction: "regenerate_chapter",
      rationale: "建议回退到本章大纲/章节生成层，而不是只重润色。当前实现先以重新生成本章承接。",
      affectedScenes,
      affectedVariants,
    };
  }

  if (/语言|文风|空泛|ai|对话|措辞|节奏拖|心理描写|重复句|修辞|叙述/.test(text)) {
    return {
      level: "edit_scene",
      confidence: "medium",
      summary: "主要问题出在润色/表达层，通常不需要整章重写。",
      recommendedAction: "revise_current_draft",
      rationale: "建议优先基于现有草稿继续修订或重润色问题片段。",
      affectedScenes,
      affectedVariants,
    };
  }

  if (/场景|第\s*\d+\s*场|动机|行为|线索|信息释放|人物|冲突/.test(text)) {
    return {
      level: "draft_scene",
      confidence: "medium",
      summary: "主要问题出在草稿场景执行层，优先修问题场景。",
      recommendedAction: "revise_current_draft",
      rationale: "建议基于审稿意见继续修订当前草稿，避免直接丢弃整章。",
      affectedScenes,
      affectedVariants,
    };
  }
  ...
  return {
    ...
    summary: "无法可靠定位问题阶段。",
    recommendedAction: "ask_author",
    rationale: "建议保留现场，由作者选择是继续修订、重生成本章，还是暂停调整输入。",
    affectedScenes,
    affectedVariants,
  };
}

function buildQualityDecision(input: { chapterTitle: string; verdict: string; issues: ReviewIssue[]; repairPlan: AutopilotRepairPlan; diagnosis?: ReviewFailureDiagnosis }): AutopilotFailureDecision {
  const materialIssues = input.issues.filter((issue) => issue.severity !== "minor");
  const shownIssues = (materialIssues.length ? materialIssues : input.issues).slice(0, 8);
  return {
    title: "需要作者决策：审稿多轮修订后仍未通过",
    summary: `《${input.chapterTitle}》已完成自动审稿与最多 ${AUTOPILOT_MAX_REVISIONS} 轮自动修订，但复审结论仍为 ${input.verdict}。`,
    attempted: [
      "已生成多版草稿并完成逐场润色。",
      `已按审稿意见自动修订 ${AUTOPILOT_MAX_REVISIONS} 轮并复审。`,
      "已保留此前定稿章节与本章已生成阶段，未把未通过正文写入终稿。",
    ],
    whyNeedsDecision: `系统定位：${input.repairPlan.summary} ${input.repairPlan.rationale}`,
    repairPlan: input.repairPlan,
    issues: shownIssues.map((issue) => ({
    ...
    options: [
      {
        ...
        label: input.repairPlan.recommendedAction === "revise_current_draft" ? "按阶段诊断继续修订" : "按审稿意见继续修订",
        description: input.repairPlan.recommendedAction === "revise_current_draft"
          ? "保留当前章节中间产物，优先修订系统定位的问题阶段；比整章重写成本更低。"
          : "从当前断点续跑，让系统基于已保留的审稿意见继续尝试。",
      },
      {
        id: "regenerate_chapter",
        label: input.repairPlan.recommendedAction === "regenerate_chapter" ? "重新生成本章（推荐）" : "重新生成本章",
        description: input.repairPlan.recommendedAction === "regenerate_chapter"
          ? "系统判断问题更靠近路线/结构层，局部修订可能不足；放弃当前失败章节中间产物后重新生成本章。"
          : "放弃当前未通过版本，重新生成这一章；仅在当前方向已经明显偏离时使用。",
      },
      {
        id: "cancel_batch",
        label: "暂停并保留现场",
        description: "停止继续往后写，保留诊断、checkpoint、已成章和中间产物，稍后再处理。",
      },
    ],
  };
}

function buildAuthorChoiceDecision(input: { chapterTitle: string; verdict: string; issues: ReviewIssue[]; repairPlan: AutopilotRepairPlan; diagnosis: ReviewFailureDiagnosis }): AutopilotFailureDecision {
  const materialIssues = input.issues.filter((issue) => issue.severity !== "minor");
  const shownIssues = (materialIssues.length ? materialIssues : input.issues).slice(0, 8);
  const authorQuestions = input.diagnosis.diagnoses.flatMap((item) => item.authorQuestion ? [item.authorQuestion] : []);
  return {
    title: "需要作者创作决策：审稿问题涉及叙事取舍",
    summary: `《${input.chapterTitle}》复审仍为 ${input.verdict}，系统已判断剩余问题不是简单排障，而是涉及创作方向选择。`,
    attempted: [
      "已完成常规自动修订与复审。",
      "已进一步诊断剩余审稿问题的类型、层级和可自动修复性。",
      "未把未通过正文写入终稿。",
    ],
    whyNeedsDecision: authorQuestions.length > 0
      ? authorQuestions.map((question) => `${question.question}\n${question.options.map((option) => `- ${option.label}：${option.consequence}`).join("\n")}`).join("\n\n")
      : input.diagnosis.rationale,
    repairPlan: input.repairPlan,
    issues: shownIssues.map((issue) => ({
      severity: issue.severity,
      problem: issue.problem,
    ...
    options: [
      {
        id: "retry_from_checkpoint",
        label: "按选定方向继续修订",
        description: "确认创作方向后，从当前断点继续，让系统按该方向修复正文。",
      },
      {
        id: "regenerate_chapter",
        label: "重新生成本章",
        description: "放弃当前未通过版本，重新生成这一章；适合你希望整体换一种执行方案。",
      },
      {
        id: "cancel_batch",
        label: "暂停并保留现场",
        description: "停止继续往后写，保留诊断、checkpoint、已成章和中间产物。",
      },
    ],
  };
}
...
function buildDuplicateDiagnosis(input: { chapterTitle: string; similarity: number }): AutopilotFailureDiagnosis {
  return {
    title: "章节重复度过高",
    summary: `《${input.chapterTitle}》与最近已成章相似度约 ${(input.similarity * 100).toFixed(0)}%。`,
    cause: "当前章节没有产生足够新的地点、信息、关系或状态变化，可能在重演已写内容。",
    impact: "系统已阻止重复章节写入终稿；此前已成章保持不变。",
    canRetryAsIs: false,
    recommendedAction: "请重新生成本章，或先调整本章路线目标后再继续。",
  };
}

function buildDuplicateDecision(input: { chapterTitle: string; similarity: number }): AutopilotFailureDecision {
  return {
    title: "需要作者决策：本章与已成章高度重复",
    summary: `《${input.chapterTitle}》与最近已成章相似度约 ${(input.similarity * 100).toFixed(0)}%，系统已阻止它写入终稿。`,
    attempted: [
      "已在提示中要求本章必须推进新状态、不得重演已成章。",
      "已完成生成与审稿，但重复度硬检查仍未通过。",
      "已保留此前定稿章节，未把重复章节写入终稿。",
    ],
    whyNeedsDecision: "重复通常不是简单润色能解决，而是本章路线、事件目标或承接点需要重新取舍。系统需要作者决定是重写本章、调整路线图，还是暂停后手动改目标。",
    issues: [
      {
        severity: "blocker",
        problem: "本章与最近已成章高度重复，缺少足够新的剧情推进。",
        evidence: `最高相似度约 ${(input.similarity * 100).toFixed(0)}%。`,
        suggestedFix: "明确本章必须产生的新地点、新信息、新关系或新状态，再重新生成。",
      },
    ],
    options: [
      {
        id: "regenerate_chapter",
        label: "重新生成本章",
        description: "让系统重新生成这一章，并更强地避开已成章内容；适合路线图方向仍正确但当前产物重复。",
      },
      {
        id: "retry_from_checkpoint",
        label: "从断点继续尝试",
        description: "保留现有断点继续执行；仅适合你确认重复判断过严，或已经在外部调整了相关输入。",
      },
      {
        id: "cancel_batch",
        label: "暂停并调整路线图",
        description: "先停下，回到路线图或本程目标里明确下一章的新事件后再继续。",
      },
    ],
  };
}
...
function recentSelectedStrategyLines(chapters: FinalChapter[], count = 5) {
  ...
  return recent
    .map((chapter, index) => {
      const rationale = chapter.selectionRationale;
      return `${index + 1}. ${chapter.title}：功能 ${rationale?.strategyPlan?.chapterFunction ?? rationale?.finalSelection.chapterFunction ?? "未知"}；策略 ${rationale?.finalSelection.selectedVariantTitle ?? chapter.sourceVariantId}；节奏备注 ${rationale?.finalSelection.rhythmNote ?? "无"}`;
    })
    .join("\n");
}

function variantExcerpt(manuscript: string) {
  const trimmed = manuscript.trim();
  const middleStart = Math.max(0, Math.floor(trimmed.length / 2) - 600);
  return {
    beginning: trimmed.slice(0, 1200),
    middle: trimmed.slice(middleStart, middleStart + 1200),
    ending: trimmed.slice(Math.max(0, trimmed.length - 1200)),
  };
}

async function planVariantStrategies(input: {
  project: NonNullable<Awaited<ReturnType<typeof getProject>>>;
  manuscriptContext: string;
  recall: string;
  chapterBoundary: string;
  selectedOption: DirectionOption;
  directionRecommendation: string;
  beatSheet: BeatSheet;
  previousChapters: FinalChapter[];
  parentArtifactId: string;
  tag: string;
}) {
...
${input.chapterBoundary}

Selected direction:
${JSON.stringify(input.selectedOption, null, 2)}

Full Muse recommendation:
${input.directionRecommendation}

Beat sheet:
${JSON.stringify(input.beatSheet, null, 2)}

Recent selected chapter strategies:
${recentSelectedStrategyLines(input.previousChapters)}

Project memory:
${recallExcerpt(input.recall)}

Plan the dynamic prose candidate strategies for this chapter. Do not write prose.`;

  const plan = await runAgent({ agent: agents.variantStrategyPlanner, prompt, maxTokens: 2200, label: `${input.tag}·规划候选策略` });
  const strategies = plan.strategies.slice(0, plan.variantCount);
  const normalized: VariantStrategyPlan = { ...plan, variantCount: strategies.length, strategies };
  const artifactId = await persistJobArtifact({
    project: input.project,
    kind: "variant_strategy",
    relativeDir: "variant-strategies",
    fileName: "variant-strategy-plan",
    title: `Variant strategy plan (${input.tag})`,
    content: JSON.stringify(normalized, null, 2),
    agentId: "variant-strategy-planner",
    stepTitle: `${input.tag} variant strategy`,
    summary: `Autopilot planned ${normalized.variantCount} dynamic variant strategies for ${input.tag}.`,
    parentArtifactId: input.parentArtifactId,
  });
  return { plan: normalized, artifactId };
}

async function selectFinalVariant(input: {
  project: NonNullable<Awaited<ReturnType<typeof getProject>>>;
  beatSheet: BeatSheet;
  strategyPlan: VariantStrategyPlan;
  editSet: EditSet;
  reviews: VariantReview[];
  previousChapters: FinalChapter[];
  parentArtifactId: string;
  ...
  const reviewByVariant = new Map(input.reviews.map((review) => [review.variantId, review]));
  const prompt = `Project: ${input.project.name}

Chapter:
${JSON.stringify({ title: input.beatSheet.chapterTitle, goal: input.beatSheet.chapterGoal, scenes: input.beatSheet.scenes.map((scene) => ({ id: scene.id, title: scene.title, purpose: scene.purpose, exitHook: scene.exitHook })) }, null, 2)}

Variant strategy plan:
...
Reviewed variants:
${JSON.stringify(
  input.editSet.variants.map((variant) => {
    ...
    return {
      ...
      title: variant.title,
      strategyPlan: variant.strategyPlan,
      manuscriptExcerpt: variantExcerpt(variant.manuscript),
      reviewVerdict: review?.verdict ?? "revise",
      issueCounts: counts,
      reviewSummary: review?.summary ?? "无审稿摘要",
      issueSummary: (review?.issues ?? []).map((issue) => `${issue.severity}: ${issue.problem}`).join("\n"),
    };
  }),
  null,
  2,
...
Select the final variant. Do not choose by order.`;

  const selection = await runAgent({ agent: agents.finalVariantSelector, prompt, maxTokens: 1800, label: `${input.tag}·选择终稿候选` });
  const chosen = input.editSet.variants.find((variant) => variant.id === selection.selectedVariantId) ?? input.editSet.variants[0];
  const normalized: FinalVariantSelection = {
    ...selection,
    selectedVariantId: chosen.id,
    selectedVariantTitle: chosen.title,
  };
  const artifactId = await persistJobArtifact({
    project: input.project,
    kind: "final_selection",
    relativeDir: "final-selections",
    fileName: "final-selection",
    title: `Final variant selection (${input.tag})`,
    content: JSON.stringify(normalized, null, 2),
    agentId: "final-variant-selector",
    stepType: "gate",
    stepTitle: `${input.tag} final selection`,
    summary: `Autopilot selected ${chosen.title} via FinalVariantSelector.`,
    parentArtifactId: input.parentArtifactId,
  });
  return { selection: normalized, artifactId, chosen };
}
...
async function appendSelectedFinalChapter(input: {
  ...
  finalSelectionArtifactId?: string;
  variant: EditedVariant;
  ...
  await persistJobArtifact({
    ...
    parentArtifactId: input.finalSelectionArtifactId ?? input.editArtifactId,
  });
...
async function autopilotReviewLoop(input: {
  ...
  // 理论上不可达（循环内已 return）。
  await saveProgress(undefined);
  return { ok: false as const, verdict: "revise", issues: [] as ReviewIssue[], repairPlan: diagnoseReviewRepairPlan([]) };
}
...
Create a chapter beat sheet for this selected direction. 大纲的每一场都要服务于"本章新进展"，不得重复"此前已写章节"里的场面或情绪转折。`;
  ...
  // 3. 动态候选策略：每章先规划 1-3 个真正服务本章功能的正文候选。
  const reusedVariantStrategy = await reloadArtifact<VariantStrategyPlan>(project, checkpoint?.variantStrategyArtifactId);
  let variantStrategyPlan: VariantStrategyPlan;
  let variantStrategyArtifactId: string;
  if (reusedVariantStrategy && checkpoint?.variantStrategyArtifactId) {
    variantStrategyPlan = reusedVariantStrategy;
    variantStrategyArtifactId = checkpoint.variantStrategyArtifactId;
  } else {
    const plannedStrategies = await planVariantStrategies({
      project,
      manuscriptContext,
      recall,
      chapterBoundary,
      selectedOption,
      directionRecommendation: directionSet.recommendation,
      beatSheet,
      previousChapters: input.existingChapters,
      parentArtifactId: outlineArtifactId,
      tag,
    });
    variantStrategyPlan = plannedStrategies.plan;
    variantStrategyArtifactId = plannedStrategies.artifactId;
    await saveStage({ variantStrategyArtifactId });
  }

  // 4. 分段写作（按动态策略生成 1-3 个候选）。
  ...
  if (reusedDraft && checkpoint?.draftArtifactId) {
    ...
    for (const request of variantStrategyPlan.strategies) {
      draftVariants.push(
    ...
    draftSet = {
      ...
      notesForEditor: [`自动模式按动态候选策略生成 ${variantStrategyPlan.strategies.length} 个草稿。${variantStrategyPlan.rationale}`],
    };
    draftArtifactId = await persistJobArtifact({
      ...
      parentArtifactId: variantStrategyArtifactId,
    });
  ...
  if (!reviewed.ok) {
    return {
      status: "quality_failed",
      chapterTitle: beatSheet.chapterTitle,
      verdict: reviewed.verdict,
      issues: reviewed.issues,
      repairPlan: reviewed.repairPlan,
  ...
  // 5.5 终稿选择器：Critic 负责找问题，Selector 负责根据本章功能、策略计划、审稿结果和近期节奏做取舍。
  const finalChoice = await selectFinalVariant({
    project,
    beatSheet,
    strategyPlan: variantStrategyPlan,
    editSet: reviewed.editSet,
    reviews: reviewed.reviews,
    previousChapters,
    parentArtifactId: reviewed.editArtifactId,
    tag,
  });

  // 5.6 近重复兜底：与最近若干已成章正文比对，过高则判为重复并停下，不把重复章交付进全文。
  ...
  for (const prior of recentChapters) {
    maxSimilarity = Math.max(maxSimilarity, manuscriptSimilarity(prior.manuscript, finalChoice.chosen.manuscript));
  }
  ...
  const finalManuscript = await appendSelectedFinalChapter({
    ...
    finalSelectionArtifactId: finalChoice.artifactId,
    variant: finalChoice.chosen,
    sourceDraftTitle: reviewed.editSet.sourceDraftTitle,
    selectionRationale: { strategyPlan: variantStrategyPlan, finalSelection: finalChoice.selection },
  });
  ...
  try {
    ...
    for (const [chapterIndex, chapter] of planned.entries()) {
      ...
      if (chapter.index <= finalizedCount) {
        ...
        await saveChapterCheckpointStage(project.id, chapter.index, { status: "completed", archivedMemory: true });
        continue;
      }
      ...
      if (result.status === "duplicate") {
        ...
        fresh.failure = {
          globalIndex: chapter.index,
          kind: "duplicate",
          category: "author_decision_required",
          reason: `相似度 ${(result.similarity * 100).toFixed(0)}%`,
          diagnosis: buildDuplicateDiagnosis({ chapterTitle: result.chapterTitle, similarity: result.similarity }),
          decision: buildDuplicateDecision({ chapterTitle: result.chapterTitle, similarity: result.similarity }),
        };
        await writeBatch(project.id, fresh);
    ...
    // quality/duplicate 已在上面标记；其余按错误内容诊断，避免把判断责任丢给用户。
    const current = await readBatch(project.id);
    if (current && current.status !== "failed") {
      const message = error instanceof Error ? error.message : String(error);
      const classified = classifyAutopilotFailureMessage(message);
      current.status = "failed";
      current.failure = {
        globalIndex: (await latestFinalChapters(project)).length + 1,
        kind: message === "Run cancelled." ? "interrupted" : "error",
 ...
 */
export async function resumeAutopilotJob(projectId: string, auto: boolean, decision?: string): Promise<{ started: boolean; reason?: string }> {
  const project = await getProject(projectId);
  if (!project) {
    throw new Error("Project not found.");
  }
  ...
  if (isGated) {
    ...
    if (!auto && decision === "cancel_batch") {
      // 暂停不是删除/清空恢复信息：保留 failure 诊断，并用 cancelled 表示“用户已暂停”。
      // findVisibleRun / resumable-autopilot 会继续展示 cancelled + failure 的诊断现场。
      batch.status = "cancelled";
      batch.autoRetryCount = AUTOPILOT_MAX_AUTO_RETRIES;
      await writeBatch(project.id, batch);
      return { started: false, reason: "已暂停自动续写" };
    }

    if (!auto && decision === "regenerate_chapter" && batch.failure) {
      delete batch.checkpoints[batch.failure.globalIndex];
    }
