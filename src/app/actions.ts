...
type ChapterPipelineResult =
  ...
  | { status: "quality_failed"; chapterTitle: string; verdict: string; issues: ReviewIssue[]; repairPlan: AutopilotRepairPlan }
...
function buildQualityDecision(input: { chapterTitle: string; verdict: string; issues: ReviewIssue[]; repairPlan: AutopilotRepairPlan }): AutopilotFailureDecision {
...
async function autopilotReviewLoop(input: {
  ...
  for (let round = startRound; round <= AUTOPILOT_MAX_REVISIONS; round += 1) {
    ...
    if (round === AUTOPILOT_MAX_REVISIONS) {
      ...
      return { ok: false as const, verdict, issues: allIssues, repairPlan: diagnoseReviewRepairPlan(allIssues) };
    }
...
Create a chapter beat sheet for this selected direction. 大纲的每一场都要服务于"本章新进展"，不得重复"此前已写章节"里的场面或情绪转折。`;
  ...
  if (!reviewed.ok) {
    return { status: "quality_failed", chapterTitle: beatSheet.chapterTitle, verdict: reviewed.verdict, issues: reviewed.issues, repairPlan: reviewed.repairPlan };
  }
    ...
    for (const [chapterIndex, chapter] of planned.entries()) {
      ...
      if (result.status === "quality_failed") {
        ...
        fresh.failure = {
          ...
          diagnosis: buildQualityDiagnosis({ chapterTitle: result.chapterTitle, verdict: result.verdict, repairPlan: result.repairPlan }),
          repairPlan: result.repairPlan,
          decision: buildQualityDecision({ chapterTitle: result.chapterTitle, verdict: result.verdict, issues: result.issues, repairPlan: result.repairPlan }),
        };
