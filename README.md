...
- 多章自动续写（Autopilot）：一次给出整体目标 + 可选逐章要点 + 章数，模型先把目标拆成可见的章节规划（`chapter_plan` 产物），再逐章自动跑完构思→大纲→写作→润色→审稿→终稿→归档；审稿不过自动按问题修订重审（默认最多 2 轮），仍不过则停在该章并报告、已成章保留；连跑期间记忆补丁自动应用，最多 10 章。
